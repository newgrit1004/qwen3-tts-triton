"""Tier 3 quality evaluation for partial patching: Base vs Hybrid vs Hybrid+Patch.

Compares CER, UTMOS, Speaker Similarity distributions to verify that
partial patching improves quality over full Hybrid while staying close to Base.

Usage:
    python -m benchmark.eval_partial --patch-range 0,20 --mode fast
    python -m benchmark.eval_partial --patch-range 0,20 --patch-range 0,24 --mode fast
"""
...
OUTPUTS_DIR = Path(__file__).parent / "output" / "eval_partial"
...
def _print_summary(
    ...
    logger.info("")
    logger.info("PARTIAL PATCHING TIER 3 EVALUATION (%s mode)", mode)
    logger.info(sep)
    logger.info(
    ...
    # Key comparison: Hybrid vs Hybrid+Patch UTMOS/CER deltas
    logger.info("")
...
def run_eval_partial(
    patch_ranges: list[tuple[int, int]],
    mode: str = "fast",
) -> dict[str, Any]:
    """Run Tier 3 evaluation: Base vs Hybrid vs Hybrid+Patch(N).
    ...
    output_dir = OUTPUTS_DIR / f"partial_{mode}"
    output_dir.mkdir(parents=True, exist_ok=True)

    # Build runner configs: Base + Hybrid + Hybrid+Patch(N) for each range
    ...
    logger.info(
        "Tier 3 partial eval (%s): Base vs %s (%d sentences, %d runs/sent)",
        mode,
        runner_names[1:],
        len(sentences),
        runs_per_sentence,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Tier 3 quality eval for partial patching"
    )
    parser.add_argument(
        "--patch-range",
        ...
        required=True,
        dest="patch_ranges",
        help="Layer range (repeatable), e.g. --patch-range 0,20.",
    )
    parser.add_argument(
        "--mode",
        choices=["fast", "full"],
    ...
    out_path = args.output or str(
        RESULTS_DIR / f"eval_partial_{args.mode}_{range_tag}.json"
    )
