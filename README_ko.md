...
**v0.3.0 배치 서빙은 이를 배치 16에서 per-sample throughput ~14배로 끌어올립니다 (단일 클립 PyTorch eager 대비).**

[English](README.md) | [벤치마크 결과](docs/benchmark_results_ko.md) | [배치 서빙](docs/batch-serving_ko.md)
...
- 🚦 **배치 서빙 (v0.3.0)** — 모든 러너의 `generate_batch()`; 배치 16에서 **per-sample throughput ~14배**, per-sample VRAM ~6–10× 감소, 품질 중립 ([문서](docs/batch-serving_ko.md))
...
**v0.3.0** 신규: 모든 러너에 `generate_batch()` 메서드가 추가되어, 스텝마다 여러 클립을 동시에 합성하고 1.7B 가중치를 읽는 비용을 배치 전체로 분산시킵니다. 동시 요청을 위한 **throughput** 레버이며, 단일 클립 latency 경로는 그대로 유지되고 공개 인터페이스도 동일한 7-모드 축을 유지합니다 (새 러너 이름 없음, v0.2.0 대비 추가된 건 `generate_batch` 하나뿐).
...
이는 단일 클립 속도 향상과 **누적(compound)** 됩니다: ~5배 단일 클립 (Hybrid vs Base) × ~3배 배치 분산 ≈ 배치 16에서 **per-sample throughput ~14배** (base 단일 클립 ~5.0초/클립 → hybrid 배치 ~0.36초/sample, RTX 5090 기준). per-sample VRAM도 ~6–10× 감소하며(hybrid 0.49 GB), Tier 3는 배치 출력이 단일 클립과 품질 동등함을 확인했습니다.

📖 per-sample VRAM · ms-per-step · 엔진 패밀리 · Tier 3 배치 패리티 → **[docs/batch-serving_ko.md](docs/batch-serving_ko.md)**
...
> [@tantara](https://github.com/tantara) 님이 [#6](https://github.com/newgrit1004/qwen3-tts-triton/issues/6)에서 기여한 결과입니다. RTX 4090 · PyTorch 2.10.0+cu128 · Triton 3.6.0 · CUDA 12.8 · 드라이버 550.144.03. bf16, batch=1, 2개 텍스트 (ko + en) — 위 RTX 5090 표와 동일 기준.

**E2E 추론**

| 모드 | 지연 (한국어) | 지연 (영어) | RTF (ko) | RTF (en) | 피크 VRAM |
|------|--------------|------------|----------|----------|-----------|
...
**커널 마이크로벤치마크** (bf16, seq_len=512, hidden=2048)

| 커널 | PyTorch (us) | Triton (us) | Speedup |
|------|--------------|-------------|---------|
...
> 🚦 **배치 서빙 (v0.3.0)**: 배치 16에서 per-sample throughput ~14배, per-sample VRAM(~6–10× 감소), ms-per-step, Tier 3 배치 패리티는 **[docs/batch-serving_ko.md](docs/batch-serving_ko.md)**에 정리되어 있습니다.
...
- [ ] Docker 배포
- [x] [ComfyUI-Qwen3-TTS-Triton](https://github.com/newgrit1004/ComfyUI-Qwen3-TTS-Triton) — ComfyUI 커스텀 노드
...
- [@tantara](https://github.com/tantara) — RTX 4090 벤치마크 결과 ([#6](https://github.com/newgrit1004/qwen3-tts-triton/issues/6))
